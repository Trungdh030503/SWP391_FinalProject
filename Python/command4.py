...
bepdientu = ['1 từ, 1 hồng ngoại', '2 từ , 1 hồng ngoại', '3 từ, 1 hồng ngoại', '2 từ, 2 hồng ngoại']

number = 1
# Open the CSV file in write mode
with open('bepdientu.csv', 'w', newline='', encoding='utf-8') as file:
    ...
    for type in range(2, 6):
        last_page = 0
        try:
            response = requests.get('https://bep247.vn/' + 'bep-dien-ket-hop-tu' + '?brand=&min=&max=&origin=&warranty=&type=' + str(type) + 'size=&power=&design=&orderby=') 
        ...
        for i in range(1, last_page + 1):
            response = requests.get('https://bep247.vn/' + 'bep-dien-ket-hop-tu' + '?brand=&min=&max=&origin=&warranty=&type=' + str(type) + 'size=&power=&design=&orderby=&page=' + str(i))
            ...
            for product in products:
                ...
                try: 
                    ...
                    writer.writerow([number, title, supplier, price, bepdientu[type-2], warranty, modelnumber, lines, new_url])
                    number += 1
                except: Exception
